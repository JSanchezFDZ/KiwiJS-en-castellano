...
PlayState.create = function () {
    ...
    this.pipes = [];
    for (var j = 0; j < 5; j++) {
        var topPipe = new Kiwi.GameObjects.Sprite(this, this.textures.pipe, j * this.pipeDistance + 200 + this.game.stage.width, -Math.random() * 300 - 400);
        this.addChild(topPipe);
        topPipe.transform.rotation += Math.PI;
        this.pipes.push(topPipe);

        var bottomPipe = new Kiwi.GameObjects.Sprite(this, this.textures.pipe, j * this.pipeDistance + 200 + this.game.stage.width, topPipe.transform.y + topPipe.height + this.pipeGap);
...
PlayState.restartGame = function () {
    ...
    for (var p in this.pipes) {
        ...
        pipe.transform.x += 0 + this.game.stage.width;
